...
import com.bjxg.tools.codegenerator.projectitem.tools.utils.nck.NckVoFileUtils;
        ...
        for (List<ClellBean> list : docList) {
            for (ClellBean fileId : list) {
                if (RowConstants.ROW_ZERO.equals(fileId.row)) {
                    ...
                    NckVoFileUtils.readNckToFile(classFile, writeFile, tableName, calssName, fileId.describe, "",
                            "", "", isEmpty, project.getNckAutoPath() + "vo/", project.getNckBasePackage(), list, project);
                }
            }
            System.out.println(
