...
public class NckDesktopFileUtils {
    ...
    private static String setListTableTrtd(File fileName, File writeFile, List<ClellBean> clLs, String path,
        ...
        for (ClellBean clell : clLs) {
            ...
            if ("vague".equals(type)) {
                ...
                fileName = new File(path + "desktopShowTableThFragment.txt");
                result = result + readToFileList(fileName, clell, path, clzss);
            } else if ("listShowVal".equals(type)) {
                if ("Date".equals(clell.type)) {
                    fileName = new File(path + "desktopShowDateFragment.txt");
                } else {
                    fileName = new File(path + "desktopShowStringFragment.txt");
                }
                    ...
                    if (read != null && read.trim().length() > 0) {
                        ...
                        if ("@describeVal".equals(read)) {
                            if ("Date".equals(clell.type)) {
                                ...
                                read = readToFileListDescribe(new File(path + "desktopTrTdFragmentSelect.txt"), clell);
                            } else {
                                read = readToFileListDescribe(new File(path + "desktopTrTdFragmentString.txt"), clell);
                            }
                        }
                        ...
                        String showImg = "";
                        if ("Y".equals(clell.isImg) || "y".equals(clell.isImg)) {
                            showImg = readToFileListShow(new File(path + "desktopShowTableImgFragment.txt"), clell, path);
                        }
